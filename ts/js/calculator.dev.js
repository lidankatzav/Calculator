...
var scientific_operators = ["**2", "**", "%", "pai", "sqrt", "root"]; // Array that represents all the IDs of the scientific operators
...
function add_digits(value) {
  ...
  if (calculation_lst.length === 0 || operators.includes(calculation_lst[last_idx]) || scientific_operators.includes(calculation_lst[last_idx])) {
...
function add_operator(value) {
  ...
  if (!operators.includes(calculation_lst[last_idx]) && !scientific_operators.includes(calculation_lst[last_idx])) {
    if (calculation_lst[last_idx].length > 1) {
      if (calculation_lst[last_idx][calculation_lst[last_idx].length - 1] === "." || calculation_lst[last_idx][calculation_lst[last_idx].length - 1] === "/") {
        return;
      }
    }
...
function add_point() {
  ...
  if (!operators.includes(calculation_lst[last_idx]) && !calculation_lst[last_idx].includes(".")) {
...
function remote_eval() {
  ...
  var URL = "https://api.mathjs.org/v4/?expr=" + encodeURIComponent(join_lst(calculation_lst));
